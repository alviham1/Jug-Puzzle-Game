...
public class Jug extends Observable{
	...
	/**
	 * Reset the amount in the jug to its initial value
	 * i.e 8 for jug0, and 0 for the other two jugs
	 */
	public void reset() {
		if (this.capacity == 8) {
			this.setAmount(8);
	...
	/**
	 * Set the amount of this to a if possible, otherwise this
	 * is unchanged.
	 * 
	 * @param a
	 *            the target amount, with 0<=a.
	 */
	...
	public int add(int addAmount) {
		...
		this.setChanged();
		this.notifyObservers("amount increased by " + addAmount);
	...
	public int remove(int removeAmount) {
		...
		this.setChanged();
		this.notifyObservers("amount reduced by " + removeAmount);
